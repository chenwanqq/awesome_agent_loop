    ...
    def run(self, query: str, messages: Optional[list[dict]] = None):
        ...
        for i in range(max_turns):
            ...
            for tool_call in message.tool_calls:
                ...
                if self.authorization_hook is not None and not self.authorization_hook(tool, tool_args):
...
class CLI:
    ...
    def interactive_authorization(self, tool: Tool, tool_args: dict) -> bool:
        ...
        if tool.name not in self.override_authorization:
            choice = Prompt.ask(f"是否授权执行工具 {tool.name}，参数 {tool_args}？", choices=["yes", "no","always"])
            if choice == "yes":
                return True
            elif choice == "always":
